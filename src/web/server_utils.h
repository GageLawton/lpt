...
inline std::string aircraft_to_json(const Aircraft* ac, uint64_t now_ms)
{
    ...
    snprintf(hdr, sizeof(hdr),
             ...
             "\"posAgeMs\":%llu,",
             icao, cs.c_str(), ac->lat, ac->lon, ac->position_valid ? "true" : "false",
             (int)ac->altitude_ft, (double)ac->groundspeed_kt, (double)ac->heading_deg,
             (int)ac->vert_rate_fpm, (unsigned)ac->msgs_rx, (unsigned long long)ac->first_seen_ms,
             (unsigned long long)ac->last_seen_ms, (unsigned long long)pos_age_ms);
    ...
    if (ac->emitter_cat != 0) {
        char tmp[32];
        snprintf(tmp, sizeof(tmp), "\"cat\":%u,", ac->emitter_cat);
        out += tmp;
    }
    if (ac->squawk != 0) {
        char tmp[32];
        snprintf(tmp, sizeof(tmp), "\"sqk\":\"%04u\",", ac->squawk);
        out += tmp;
    }
    if (ac->emergency_state != 0) {
        char tmp[32];
        snprintf(tmp, sizeof(tmp), "\"emrg\":%u,", ac->emergency_state);
        out += tmp;
    }
    ...
    for (int i = 0; i < ac->trail_len; i++) {
        ...
        char pt[64];
        snprintf(pt, sizeof(pt), "%s{\"lat\":%.6f,\"lon\":%.6f}", i > 0 ? "," : "",
                 ac->trail[idx].lat, ac->trail[idx].lon);
        out += pt;
    }
    out += "]}";
    return out;
